...
def create_features(df):
    ...
    # 3-year rolling averages (fixed window=3)
...
# Final features (add Is_Rookie)
features = [
    'Age', 'Career_G', 'Career_HR', 'Career_SB',
    'OBP_3yr', 'SLG_3yr', 'WAR', 'Is_Rookie'
]
target = 'Next_WAR'
...
# Save artifacts + league averages
joblib.dump(model, "war_predictor_model_CORRECTED.pkl")
joblib.dump(scaler, "scaler_war_CORRECTED.pkl")
joblib.dump(
    {'league_avg_obp': league_avg_obp, 'league_avg_slg': league_avg_slg},
    "league_avgs_CORRECTED.pkl"
)

print("Model trained successfully with rookie support!")