...
model = joblib.load("war_predictor_model_IMPROVED.pkl")
scaler = joblib.load("scaler_war_IMPROVED.pkl")
...
def prepare_prediction_data(df):
    ...
    # 3-year averages (excluding injury seasons)
    three_year_avg = df[df['AB'] >= 200].groupby('Player').apply(
    ...
    latest_data['Injured'] = (latest_data['AB'] < 200).astype(int)
    latest_data['AB_Drop'] = (latest_data.groupby('Player')['AB'].shift(1) - latest_data['AB']) / latest_data.groupby('Player')['AB'].shift(1)
    latest_data['Seasons_Since_Injury'] = latest_data.groupby('Player')['Injured'].cumsum()
    
    # Age for next season (2025)
    latest_data['Age'] = latest_data['Age'] + 1
    
    return latest_data[['Player', 'Age', 'Career_G', 'Career_HR', 'Career_SB', 'OBP_3yr', 'SLG_3yr', 'WAR', 'Injured', 'AB_Drop', 'Seasons_Since_Injury']]
...
while True:
    ...
    features = ['Age', 'Career_G', 'Career_HR', 'Career_SB', 'OBP_3yr', 'SLG_3yr', 'WAR', 'Injured', 'AB_Drop', 'Seasons_Since_Injury']
